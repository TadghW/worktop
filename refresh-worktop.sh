#!/bin/bash 
sudo podman build . --tag=worktop-image
...
sudo podman run -d --name worktop-container "${mounts[@]}" --publish 0.0.0.0:22222:22/tcp --hostname worktop --replace worktop-image
